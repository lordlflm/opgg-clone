...
// async fn get_matches(puuid: String, region: String) -> Result</* HAHAH */> {
//   let mut response = 
//   let matches_id = get_matches_id(puuid, region).await;
...
async fn get_matches_id(puuid: String, region: String) -> Result<Vec<String>, String> {
  ...
  match scuttle::get_matches_from_puuid(puuid, region).await {
    Ok(matches) => {
      if matches.len() == 0 {
        return Ok(response);
      }
